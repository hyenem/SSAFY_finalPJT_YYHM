    ...
    <div class="request">
        ...
        <div class="alarmlist">
            <div v-for="pt in filteredPtList">
                ...
                <div v-if="pt.requestState > 4" class="btn-list">
                    <button @click="checkResponse(pt.diaryId)">반영하기</button>
                </div>
                <div v-else class="alarm-info">트레이너의 응답을 기다리는 중입니다.</div>
            </div>
        </div>
    </div>
</template>
...
const checkResponse = function(diaryId){
    const check = confirm("해당 응답을 반영하시겠습니까?")
    if(check){
        axios.patch("http://localhost:8080/pt/check", {
            diaryId : diaryId
        }).then(()=>{
            router.go(0)
        })
